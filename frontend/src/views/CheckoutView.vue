...
async function submit() {
  ...
  error.value = ''
  try {
    const order = await meApi.createOrder({
      sku_id: sku.value.id,
      quantity,
    ...
    router.push(
      method.value === 'online'
        ? `/pay/${order.order_no}`
        : `/me/orders/${order.order_no}`,
    )
  } catch (e) {
    error.value = apiMessage(e, '下单失败，请检查余额或库存')
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="section container checkout-page">
    <div class="page-title">
      <div>
        <h1>确认订单</h1>
        <p>登录用户可选择余额或在线支付，支付成功后即时交付 CDK。</p>
      </div>
    </div>
    ...
    <div class="checkout-grid">
      <div class="checkout-main">
        <div class="form-section">
          ...
          <div class="payment-options">
            <button
              type="button"
              :class="{ selected: method === 'balance' }"
              @click="method = 'balance'"
            >
              <WalletCards />
              <span>
                <strong>余额支付</strong>
                <small>当前可用 {{ balanceText }}，扣款后立即交付</small>
              </span>
            </button>
            <button
              type="button"
              :class="{ selected: method === 'online' }"
              @click="method = 'online'"
            >
              <CreditCard />
              <span>
                <strong>在线支付</strong>
                <small>创建支付单，付款成功后交付</small>
              </span>
            </button>
          </div>
        </div>
      ...
      <aside class="order-summary">
        ...
        <dl>
          ...
          <div>
            <dt>支付方式</dt>
            <dd>{{ method === 'balance' ? '余额支付' : '在线支付' }}</dd>
          </div>
          <div>
            <dt>交付方式</dt>
            <dd>订单内展示</dd>
