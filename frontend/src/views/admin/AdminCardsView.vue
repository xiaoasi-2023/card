...
const cards=ref<any[]>([]),products=ref<Product[]>([]),skus=ref<Sku[]>([]),skuId=ref<number>(),content=ref(''),batch=ref<any>(),error=ref(''),query=ref('')
async function load(){try{cards.value=rows(await adminApi.list('cards',{q:query.value}));products.value=rows(await adminApi.products());skus.value=rows(await adminApi.skus())}catch(e){error.value=apiMessage(e)}}
